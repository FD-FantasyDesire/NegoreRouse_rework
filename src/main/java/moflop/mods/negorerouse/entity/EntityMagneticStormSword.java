 ...
 */
public class EntityMagneticStormSword extends EntityPhantomSwordExBase {
    public EntityMagneticStormSword(World par1World) {
        super(par1World);
    }

    public EntityMagneticStormSword(World par1World, EntityLivingBase entityLiving, float AttackLevel) {
        super(par1World, entityLiving, AttackLevel);
    }

    public EntityMagneticStormSword(World par1World, EntityLivingBase entityLiving, float AttackLevel, float roll) {
        super(par1World, entityLiving, AttackLevel, roll);
    }

    private static final DataParameter<Boolean> Burst = EntityDataManager.<Boolean>createKey(EntityMagneticStormSword.class, DataSerializers.BOOLEAN);
    @Override
    ...
    protected void attackEntity(Entity target) {
        ...
        if(!this.world.isRemote){
            ...
            if(!blade.isEmpty() && target instanceof EntityLivingBase && thrower != null && thrower instanceof EntityLivingBase){
                ...
                if(getBurst()){
                    ((EntityLivingBase)target).addPotionEffect(new PotionEffect(MobEffects.WEAKNESS, 20 * 5, 5));
